...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Sign in — AquaIntel" }] }),
  beforeLoad: async () => {
    ...
    <div className="relative flex min-h-screen items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="glass-strong rounded-2xl p-8 glow-aurora">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Sign in to your municipal dashboard</p>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" required className="mt-1.5 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2.5 text-sm outline-none focus:border-aqua/60" />
            </div>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            New to AquaIntel? <Link to="/register" className="text-aqua hover:underline">Create account</Link>
          </p>
        </div>
      </div>
    </div>
